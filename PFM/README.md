...
Our implementation of PFM is adpted from [ZipIt!](https://github.com/gstoica27/ZipIt), where a highly flexible implementation of model merging is provided based on graph representations of models. We modify the original repo to support our models, datasets, and the preserve-first merging framework. Below is content of the original `README.md` which provides detailed descriptions of the original code. Note that there is **No Need** to run their installation code. It's already contained in ours.
